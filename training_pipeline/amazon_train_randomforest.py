...
CSV_PATH = "../data/AMZN_yfinance_daily_2023-11-03_to_2024-11-02.csv"
...
def read_data(file_path: str) -> pd.DataFrame:
    ...
    # Filtrar datos para evitar valores anómalos
...
def train_best_model(X_train, X_val, y_train, y_val, best_params) -> None:
    with mlflow.start_run(run_name="Mejor modelo Random Forest"):
        ...
        with open("models/amazon_random_forest_model.pkl", "wb") as f_model:
            pickle.dump(rf_model, f_model)
        mlflow.log_artifact("models/amazon_random_forest_model.pkl", artifact_path="model")
...
def register_model():
    ...
    result = mlflow.register_model(
        model_uri=run_uri,
        name="amazon-stock-model-randomforest-prefect"
    )
...
def main_flow() -> None:
    ...
    mlflow.set_experiment(experiment_name="amazon-stock-model-randomforest-prefect")
